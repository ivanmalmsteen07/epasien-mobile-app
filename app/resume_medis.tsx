...
export default function ResumeMedisSelectionScreen() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (res.data.status === 'success') {
        // Gabungkan dengan diri sendiri
        const selfRes = await api.get('/get_my_bookings.php?limit=1'); // Hack to get profile
    ...
    try {
      ...
      if (res.data.status === 'success') {
        if (res.data.is_active) {
          router.push({
            pathname: '/resume_medis_history',
            params: { no_rkm_medis: patient.no_rkm_medis, nm_pasien: patient.nm_pasien }
          });
        } else {
          router.push({
            pathname: '/resume_medis_consent',
            params: { no_rkm_medis: patient.no_rkm_medis, nm_pasien: patient.nm_pasien, from: '/resume_medis_history' }
          });
        }
      }
    } catch (e) {
    ...
    <View style={styles.container}>
      ...
      <LinearGradient colors={['#004d40', '#00796b', '#1abc9c']} style={styles.header}>
        <RNView style={styles.headerRow}>
          ...
          <RNView style={styles.headerTitleBox}>
            <Text style={styles.headerTitle}>Resume Medis</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
        <Animated.View entering={FadeInDown} style={styles.infoBox}>
          <MaterialCommunityIcons name="shield-lock-outline" size={30} color="#00796b" />
          <Text style={styles.infoText}>
            Data Rekam Medis bersifat sangat rahasia. Anda akan diminta melakukan verifikasi identitas (Selfie & Tanda Tangan) sebelum mengakses data ini.
          </Text>
        </Animated.View>
