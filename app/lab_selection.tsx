...
export default function LabSelectionScreen() {
    ...
    try {
      ...
      if (res.data.status === 'success') {
        if (res.data.is_active) {
          router.push({
            pathname: '/lab_history',
            params: { no_rkm_medis: patient.no_rkm_medis, nm_pasien: patient.nm_pasien }
          });
        } else {
          router.push({
            pathname: '/resume_medis_consent',
            params: { no_rkm_medis: patient.no_rkm_medis, nm_pasien: patient.nm_pasien, from: '/lab_history' }
          });
        }
      }
    } catch (e) {
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
        <Animated.View entering={FadeInDown} style={styles.infoBox}>
          <MaterialCommunityIcons name="flask-outline" size={30} color="#00796b" />
          <Text style={styles.infoText}>
            Pilih profil di bawah ini untuk melihat riwayat hasil pemeriksaan laboratorium yang telah divalidasi.
          </Text>
        </Animated.View>
