...
import { StyleSheet, TouchableOpacity, ScrollView, Dimensions, StatusBar, ActivityIndicator, View, Platform } from 'react-native';
...
export default function RadiologiSelectionScreen() {
    ...
    try {
      ...
      if (res.data.status === 'success') {
        if (res.data.is_active) {
          router.push({
            pathname: '/radiologi_history',
            params: { no_rkm_medis: patient.no_rkm_medis, nm_pasien: patient.nm_pasien }
          });
        } else {
          router.push({
            pathname: '/resume_medis_consent',
            params: { no_rkm_medis: patient.no_rkm_medis, nm_pasien: patient.nm_pasien, from: '/radiologi_history' }
          });
        }
      }
    } catch (e) {
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
        <Animated.View entering={FadeInDown} style={styles.infoBox}>
          <MaterialCommunityIcons name="radiology-box" size={30} color="#00796b" />
          <Text style={styles.infoText}>
            Pilih profil di bawah ini untuk melihat riwayat hasil pemeriksaan Radiologi (Rontgen, CT Scan, USG, dsb).
          </Text>
        </Animated.View>
